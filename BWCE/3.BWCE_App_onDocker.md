...
#### 2.5.1 Provider Application Docker Build
1. Dockerfile 생성
```
...
```
3. Docker image 결과 확인
```bash
$ docker images
```


#### 2.5.2 Consumer Application Docker Build
1. Dockerfile 생성
```
...
2. Docker Build
```bash
$ docker build -t consumer:latest -t consumer:1.0 -f Dockerfile_Provider .
```
3. Docker image 결과 확인
```bash
$ docker images
...
#### 2.6.1 Provider Application Docker Run

1. docker run 을 통한 container 실행
...
2. docker ps를 통한 container 실행 확인
```bash
$ sudo docker ps
...
#### 2.6.2 Consumer Application Docker Run

1. docker run 을 통한 container 실행

```bash
$ sudo docker run -d -p 7000:7000 --name consumer_1.0 consumer:latest
```
